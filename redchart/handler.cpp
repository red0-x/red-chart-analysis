...
void CfgHandler::set(const std::string&ky, const std::string&val){
    cfg[ky] = val;
}

bool CfgHandler::save(const std::string&cfgPath) const{
    std::ifstream in(cfgPath);
    std::vector<std::string> lines;
    std::unordered_map<std::string,bool> written;
    if (in.is_open()){
        std::string lnn;
        while(std::getline(in, lnn)){
            if (lnn.empty() || lnn[0] == '#'){
                lines.push_back(lnn);
                continue;
            }
            ...
            std::string key = lnn.substr(0, eq);
    ...
    for (const auto& [k,v] : cfg){
        if (!written[k]) lines.push_back(k + "=" + v);
    }
    std::ofstream out(cfgPath);
    if (!out.is_open()){
        std::cerr << "failed writing " << cfgPath << "." << std::endl;
        return false;
    }
...
bool CsvHandler::load(const std::string&csvPath){
    ...
    std::getline(red0xx, lnn); // skip description line
    std::getline(red0xx, lnn); // skip column header line
    int lines = 0;
